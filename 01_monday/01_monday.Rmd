...
---
COMMENTS: This template is to be used for the HANDOUTS of the BARCELONA SUMMER SCHOOL OF DEMPGRAPHY
AUTHOR: Francisco Villavicencio
LAST UPDATE: 07 Feb 2018
INSTRUCTIONS:
 - The template is set up to be complied into PDF, but it also works for HTML documents
 - If you are willing to COMPILE into PDF, you may
    1) In the 'title' section above, replace by the NUMBER and NAME of your MODULE, and the NUMBER and TITLE of your SESSION.
    2) In the 'header-includes' section above, replace by the NUMBER and NAME of your MODULE, and the TITLE of your SESSION (I'd rather NOT include the NUMBER of SESSION here, otherwise too much text).
    3) In the 'header-includes' section, replace by your name (initials of first name + full surname).
    4) In the Latex code below, replace by your name, email address and correct date.
 - If you are willing to COMPILE into HTML, the current title, author, date and headers will not show up. 
   Therefore, you may:
    1) Replace the current title by the following:
    title: |
      | Barcelona Summer School of Demography
      | Module X. Name of your module
    2) Below the title, add the following
    subtitle: Number and Title of your session
    author: Name and Surname - Email address
    date: DAY MONTH YEAR
WARNING: When modifying all these data, be careful of respecting all the vertical and horizontal spaces, so all the handouts have the same layout.
---
...
\textbf{Dataframe:} A `data.frame` is a table where the columns are variables and the rows are observation. Each column/variable can be of different types (e.g. factors, numeric, etc.). Can be created with `data.frame()`. `tibble` is another kind of `data.frame` we'll introduce.

\textbf{Vector:} A combination of elements. Can be created with `c()`, `seq()`, `rep()`.

\textbf{Matrix:} A matrix, is a two-dimensional object where a variable, generally numeric (e.g. birth counts), is arranged into rows and columns. Can be created with `matrix()`.

\textbf{List:} A list can contain elements of different types. Can be created with `list()`.
...
* Calculate age-specific fertility rates from 1950 until 2014 (annually) from age 12 to 55. Skip the year 1975!
